...
def index():
    try:
        ...
        cities = list(set(l.city for l in active_lots))
        total_cities = len(cities)
        active_bookings = Reservation.query.filter_by(status='active').count()
    except Exception:
        total_lots = 0; total_slots = 0; available_slots = 0; total_cities = 0; active_bookings = 0
    return render_template('index.html',
        total_lots=total_lots, total_slots=total_slots,
...
def api_live_stats():
    try:
        ...
        return jsonify({
            'total_lots': total_lots, 'total_slots': total_slots,
            'available_slots': available_slots, 'total_cities': total_cities,
            'active_bookings': active_bookings, 'ok': True
        })
    except Exception as e:
